...
<div class="main-wrapper">
        ...
        <div class="container">
            ...
            <div class="text-center">
                ...
                <p class="lead mb-0 opacity-75">
                    Menampilkan {{ $inventarisasi->total() }} koleksi {{ strtolower($jenisNama) }} dari database museum
                </p>
            </div>
        </div>
    </header>
        ...
        <div class="row g-4" id="koleksiGrid">
            ...
            <div class="col-lg-3 col-md-4 col-sm-6 koleksi-item">
                <div class="card koleksi-card h-100">
                    @if($item->registrasi && $item->registrasi->foto)
                        <img src="{{ asset('storage/' . $item->registrasi->foto) }}" class="card-img-top" alt="{{ $item->registrasi->nama_koleksi }}">
                    @else
                    ...
                    <div class="card-body d-flex flex-column">
                        <h6 class="card-title fw-bold">{{ $item->registrasi->nama_koleksi ?? 'Nama Tidak Tersedia' }}</h6>
                        ...
                        <div class="mt-auto">
                            <div class="d-flex flex-wrap gap-1 mb-3">
                                ...
                                @if($item->kondisi_fisik)
                                    <span class="badge {{ $item->kondisi_fisik === 'baik' ? 'bg-success' : ($item->kondisi_fisik === 'rusak' ? 'bg-warning' : 'bg-danger') }}">
            ...
            <div class="col-12">
                <div class="text-center py-5">
                    ...
                    <p class="text-muted">Belum ada koleksi {{ strtolower($jenisNama) }} yang tersedia dalam database.</p>
    ...
    <footer class="bg-dark text-white text-center py-4 mt-5">
        <div class="container">
            <h6 class="fw-bold">MUSEUM TANAH DAN PERTANIAN</h6>
            <p class="small mb-0 opacity-75">© {{ date('Y') }} - Semua Hak Dilindungi</p>
        </div>
    </footer>
</div>
    ...
    document.getElementById('kondisiFilter').addEventListener('change', function() {
        ...
        cards.forEach(card => {
            ...
            badges.forEach(badge => {
                if (badge.textContent.toLowerCase().includes(selectedKondisi) || selectedKondisi === '') {
                    hasCondition = true;
                }
            });
